...
sub _setupCallbacks {
	...
	$$self{_WINDOWPCC}{entryData} -> signal_connect( 'button_release_event' => sub {
		...
		foreach my $uuid ( keys %{ $$self{_RUNNING} } ) {
			...
			$$self{_RUNNING}{$uuid}{'terminal'}{_GUI}{_VTE} -> feed_child( $text );
	...
	$$self{_WINDOWPCC}{cbShowMultiText} -> signal_connect( 'toggled' => sub {
		if ( $$self{_WINDOWPCC}{cbShowMultiText} -> get_active ) {
			...
			$$self{_WINDOWPCC}{main}			-> set_resizable( 0 );
			$$self{_WINDOWPCC}{vboxMultiText}	-> hide_all;
...
sub _updateGUI {
	...
	foreach my $cluster ( keys %{ $$self{_CLUSTERS} } ) { $$self{_WINDOWPCC}{comboTerminals} -> remove_text( 0 ); }
	...
	foreach my $uuid ( keys %{ $$self{_CLUSTERS}{$cluster} } ) {
		$$self{_RUNNING}{$uuid}{'terminal'}{_PROPAGATE} = ! $$self{_WINDOWPCC}{cbPreventSingle} -> get_active unless ! $$self{_WINDOWPCC}{main} -> visible;
	}
